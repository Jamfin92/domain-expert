...
Three commits, not pushed: `c3863f3` → `4e94613` → `db3401a`.
`master` is now **10 ahead** of `origin/master` (7 + 3).
...
**`apps/server/test/store.test.ts`** (new, 20 tests). B5a, B12, B15, B18, B19,
B22, B23, B26 plus the envelope round trip, the read-path refusals, and four
controls.
...
29 files, exit 0. New total **386** (+21: 20 in `store.test.ts`, 1 in
...
| B2 | **PASS** | `PSQ_NO_CORPUS=1 pnpm test` → **328 passed \| 58 skipped (386)**, 0 failed, exit 0. **Skipped held at 58**, matching G-a. The private corpus config is intact. |
| B3 | **PASS** | `pnpm test` → **386 passed (386)**, 30 files, 0 failed, exit 0. |
...
| B27 | **PASS** | `git diff 18b8143 HEAD -- packages/extract/test/digest.test.ts` is a single 3-line hunk at `@@ -17,9 +17,9 @@`, replacing **lines 20-22** with the `Parameters<typeof actual.digestOf>` wrapper. Nothing else. The two A6 rename assertions are still at `:126` and `:141`, verbatim; G-a's "path-awareness lives in exactly two assertions" holds. (The plan predicted lines 20-**23**; the change is 20-22. Cosmetic, recorded for honesty.) |
